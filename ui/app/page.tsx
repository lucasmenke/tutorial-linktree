...
function LinkCard({
  ...
  return (
    <a
      href={url}
      className="flex items-center p-1 w-full rounded-md hover:scale-105 transition-all border border-gray-300 mb-2 bg-white max-w-3xl"
    >
...
export default async function Home() {
  // -- Fetch data from local json file for developing and switch to api for production
  //const response: DTO = await fetchProfile(1);
  const response: DTO = data;
  ...
  return (
    <div className="flex flex-col items-center mx-auto w-full mt-16 px-8">
      <Image
        ...
        height={96}
      />
      <h1 className="font-bold mt-4 mb-8 text-xl">{response.data.name}</h1>
      {response.data.links.map((link) => (
        <LinkCard key={link.url} {...link} />
      ))}
      <div className="gap-4 flex items-center mt-8">
        {response.data.socials.map((link) => {
